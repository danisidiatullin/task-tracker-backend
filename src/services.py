...
async def _task_selector(task_id: int, user: schemas.User, db: orm.Session):
    task = db.query(models.Task).filter_by(user_id=user.id).filter(models.Task.id == task_id).first()
...
async def update_task(task_id: int, task: schemas.TaskCreate, user: schemas.User, db: orm.Session):
